            ...
            try {
                ...
                const options = { 
                    ...
                    requestOptions: proxyUtils.isMaskingActive() ? {
                        ...
                        timeout: 30000,
                        agent: false // Désactiver l'agent par défaut
                    } : {}
                };
...
async function downloadYouTube(url, format, quality, title) {
    ...
    // Options ytdl manquantes dans le scope global
    const ytdlOptions = {
        requestOptions: {
            headers: proxyUtils.getMaskedHeaders(),
            timeout: 90000,
    ...
    // Stratégie de téléchargement ultra-renforcée avec cookies
    const baseYtdlOptions = {
        requestOptions: {
            headers: proxyUtils.getMaskedHeaders(),
            timeout: 90000, // Timeout plus long
            maxRedirects: 10,
            maxRetries: 3,
            retryDelay: 2000,
            agent: false // Important pour éviter la détection
        },
        highWaterMark: 512 * 1024, // Chunks plus petits pour moins de suspicion
        quality: 'highest',
        filter: format => format.container === 'mp4'
    };

    // Log du statut des cookies
    if (proxyUtils.hasCookies()) {
        proxyUtils.logMasking('cookies YouTube', 'Session authentifiée disponible');
    } else {
        proxyUtils.logMasking('cookies YouTube', 'Aucune session - mode anonyme');
    }

    // Rotation des stratégies selon les tentatives
    const strategies = [
        { ...baseYtdlOptions }, // Stratégie de base
        { 
            ...baseYtdlOptions, 
            requestOptions: { 
                ...baseYtdlOptions.requestOptions, 
                headers: { ...proxyUtils.getMaskedHeaders(), 'Range': 'bytes=0-' }
            }
        }, // Avec Range header
        { 
            ...baseYtdlOptions, 
        ...
        } // Qualité réduite en dernier recours
    ];
    ...
    if (format === 'audio') {
        ...
        for (let strategyIndex = 0; strategyIndex < strategies.length && !success; strategyIndex++) {
            ...
            await new Promise((resolve, reject) => {
                ...
                // Si c'est une erreur de robot, essayer de rafraîchir les cookies
                if (error.message.includes('confirm you are not a robot') || 
                    error.message.includes('Sign in to confirm')) {
                    
                    if (strategyIndex === 0 && proxyUtils.hasCookies()) {
                        proxyUtils.logMasking('tentative refresh cookies', 'Erreur robot détectée');
        ...
        try {
            // Appliquer le masquage pour la vidéo aussi
