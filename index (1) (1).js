...
// Fallback downloader pour Render
let YtDlpWrap;
try {
    YtDlpWrap = require('yt-dlp-wrap').default;
} catch (e) {
    console.log('yt-dlp-wrap non disponible, utilisation de ytdl-core uniquement');
}
...
const RENDER_CONFIG = {
    ...
    maxFileSize: 20 * 1024 * 1024, // 20MB max pour Render
    fallbackToYtDlp: true
};

// Agent personnalisé optimisé pour Render
...
async function downloadYouTube(url, format, quality, title) {
    ...
    const ytdlOptions = {
        agent: ytdlAgent,
        requestOptions: {
            timeout: RENDER_CONFIG.requestTimeout,
            headers: {
                'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
                'Accept': '*/*',
                'Accept-Language': 'en-US,en;q=0.9',
        ...
        highWaterMark: 256 * 1024, // Chunks plus petits pour Render
        retries: RENDER_CONFIG.maxRetries
    };

    // Essayer d'abord avec ytdl-core, puis fallback vers youtube-dl-exec
    try {
        return await downloadWithYtdlCore(url, format, quality, tempPath, ytdlOptions);
    } catch (error) {
        console.log('ytdl-core a échoué, tentative avec yt-dlp...');
        if (YtDlpWrap && RENDER_CONFIG.fallbackToYtDlp) {
            return await downloadWithYtDlExec(url, format, quality, tempPath);
        }
        throw error;
    }
}

async function downloadWithYtDlExec(url, format, quality, tempPath) {
    const outputPath = format === 'audio' ? `${tempPath}.%(ext)s` : `${tempPath}.%(ext)s`;
    
    try {
        const ytDlpWrap = new YtDlpWrap();
        
        const options = [
            '--format', format === 'audio' ? 'bestaudio[ext=m4a]/bestaudio' : 'best[height<=480]/best',
            '--output', outputPath,
            '--max-filesize', '20m',
            '--no-playlist',
            '--prefer-free-formats'
        ...
        if (format === 'audio') {
            options.push('--extract-audio', '--audio-format', 'mp3', '--audio-quality', '96K');
        }

        await Promise.race([
            ytDlpWrap.execPromise([url, ...options]),
            new Promise((_, reject) => 
                setTimeout(() => reject(new Error('Timeout yt-dlp')), RENDER_CONFIG.downloadTimeout)
            )
        ]);

        // Trouver le fichier créé
        const files = fs.readdirSync(path.dirname(tempPath));
        const downloadedFile = files.find(file => file.startsWith(path.basename(tempPath)));
        
        if (!downloadedFile) {
            throw new Error('Fichier non créé');
        }

        const finalPath = path.join(path.dirname(tempPath), downloadedFile);
        ...
        throw new Error(`Échec yt-dlp: ${error.message}`);
    }
}

async function downloadWithYtdlCore(url, format, quality, tempPath, ytdlOptions) {
