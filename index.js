...
// =============================================
// EXTENSION YOUTUBE DOWNLOADER PRO AVEC MASQUAGE
// =============================================
...
const ProxyUtils = require('./proxy_utils');
ffmpeg.setFfmpegPath(ffmpegPath);

// Initialiser le système de masquage
const proxyUtils = new ProxyUtils();
...
async function searchYouTube(query) {
    // Appliquer le masquage intelligent si sur Render
    if (proxyUtils.isMaskingActive()) {
        proxyUtils.logMasking('recherche YouTube', `Query: ${query}`);
        await proxyUtils.addIntelligentDelay('search');
    }
    ...
    while (attempts < maxAttempts && results.length === 0) {
        try {
            // Capturer les erreurs de console pour ignorer shortsLockupViewModel
            const originalConsoleError = console.error;
            const suppressedErrors = [];
            
            console.error = (...args) => {
                const errorMessage = args.join(' ');
                if (errorMessage.includes('shortsLockupViewModel') || 
                    errorMessage.includes('parseItem') ||
                    errorMessage.includes('ytsr/dumps')) {
            ...
            try {
                ...
                // Utiliser l'agent ytdl si disponible
                const options = { 
                ...
                // Ajouter les options de requête avec masquage si nécessaire
                if (proxyUtils.isMaskingActive()) {
                    options.requestOptions = {
                        headers: proxyUtils.getMaskedHeaders(),
                        timeout: 30000,
                        agent: proxyUtils.getYtdlAgent() || false
                    };
                    
                    // Ajouter les cookies à la recherche aussi pour cohérence
                    if (proxyUtils.hasCookies()) {
                        const cookieString = proxyUtils.youtubeCookies.map(c => `${c.name}=${c.value}`).join('; ');
                        options.requestOptions.headers['Cookie'] = cookieString;
                        proxyUtils.logMasking('cookies recherche', 'Cookies ajoutés à la recherche');
                    }
                }
                ...
                if (results.length > 0) {
                    proxyUtils.logMasking('recherche réussie', `${results.length} résultats (erreurs supprimées: ${suppressedErrors.length})`);
                    break;
                }
            } catch (searchError) {
                ...
                // Si c'est une erreur de parsing, essayer de continuer quand même
                if (searchError.message.includes('shortsLockupViewModel') || 
                    searchError.message.includes('parseItem')) {
                    proxyUtils.logMasking('erreur parsing ignorée', searchError.message);
                    
                    // Essayer de récupérer des résultats partiels
                    if (searchError.results && Array.isArray(searchError.results)) {
                        results = searchError.results.filter(item => 
                            item && item.title && item.url && item.type === 'video'
                        );
                        if (results.length > 0) {
                            proxyUtils.logMasking('récupération partielle', `${results.length} résultats récupérés`);
                            break;
                        }
                    }
                }
            ...
            attempts++;
            proxyUtils.logMasking(`échec recherche (tentative ${attempts})`, error.message);
            
            // Si c'est juste une erreur de shortsLockupViewModel, ne pas abandonner
            if (error.message.includes('shortsLockupViewModel') && attempts < maxAttempts) {
                await proxyUtils.addIntelligentDelay('search');
                continue;
            }
            ...
            if (attempts < maxAttempts) {
                ...
                // En dernier recours, essayer une recherche simplifiée
                try {
                    proxyUtils.logMasking('tentative recherche simplifiée', 'Dernière chance');
                    const simpleResults = await ytsr(query, { 
                        limit: 5,
                        requestOptions: proxyUtils.isMaskingActive() ? {
                            headers: proxyUtils.getMaskedHeaders(),
                            timeout: 20000
                        } : {}
                    });
                    
                    if (simpleResults && simpleResults.items) {
                        results = simpleResults.items.filter(item => 
                            item && item.title && item.url && item.type === 'video'
                        );
                        if (results.length > 0) {
                            proxyUtils.logMasking('recherche simplifiée réussie', `${results.length} résultats`);
                            break;
                        }
                    }
                } catch (finalError) {
                    proxyUtils.logMasking('recherche simplifiée échouée', finalError.message);
                }
                
                throw new Error('Recherche YouTube échouée après plusieurs tentatives');
            }
        }
    }
...
async function downloadYouTube(url, format, quality, title) {
    ...
    // Appliquer le masquage si sur Render
    if (proxyUtils.isMaskingActive()) {
        proxyUtils.logMasking('téléchargement YouTube', `${format} - ${title.substring(0, 50)}`);
        // Délai avant téléchargement pour éviter la détection
        const delayMs = format === 'audio' ? 
            Math.random() * 1500 + 500 :  // Audio: 0.5-2s
            Math.random() * 2000 + 1000;   // Vidéo: 1-3s
        await proxyUtils.addRandomDelay(delayMs, delayMs + 500);
    }

    // Options ytdl avec agent cookies
    const baseYtdlOptions = {
        requestOptions: {
            timeout: 90000,
            maxRedirects: 10,
            maxRetries: 3,
    ...
    // Ajouter l'agent ytdl si disponible
    if (proxyUtils.hasYtdlAgent()) {
        baseYtdlOptions.agent = proxyUtils.getYtdlAgent();
        proxyUtils.logMasking('agent utilisé', 'Agent ytdl avec cookies');
    } else {
        // Fallback vers headers masqués
        baseYtdlOptions.requestOptions.headers = proxyUtils.getMaskedHeaders();
        baseYtdlOptions.requestOptions.agent = false;
        proxyUtils.logMasking('fallback headers', 'Pas d\'agent ytdl');
    }

    // Log du statut de l'agent ytdl
    if (proxyUtils.hasYtdlAgent()) {
        proxyUtils.logMasking('agent ytdl', 'Agent avec cookies disponible');
    } else if (proxyUtils.hasCookies()) {
        proxyUtils.logMasking('cookies', 'Cookies disponibles mais agent échoué');
    } else {
        proxyUtils.logMasking('authentification', 'Mode anonyme');
    }

    // Rotation des stratégies selon les tentatives
    const strategies = [
        { ...baseYtdlOptions }, // Stratégie de base avec agent
        { 
            ...baseYtdlOptions, 
            requestOptions: { 
                ...baseYtdlOptions.requestOptions,
                timeout: 60000 // Timeout réduit
            }
        }, // Avec timeout réduit
        { 
            ...baseYtdlOptions, 
            ...
            agent: null, // Sans agent en dernier recours
            requestOptions: {
                headers: proxyUtils.getMaskedHeaders(),
                timeout: 45000,
                agent: false
            }
        } // Qualité réduite en dernier recours sans agent
    ];

    if (format === 'audio') {
        filePath = `${tempPath}.mp3`;
        
        // Système de téléchargement avec stratégies multiples
        let success = false;
        let lastError = null;
        
        for (let strategyIndex = 0; strategyIndex < strategies.length && !success; strategyIndex++) {
            try {
                const currentStrategy = strategies[strategyIndex];
                proxyUtils.logMasking(`tentative stratégie ${strategyIndex + 1}`, 'téléchargement audio');
                
                // Délai spécial avant chaque tentative
                if (strategyIndex > 0) {
                    await proxyUtils.addIntelligentDelay('download');
                }
                
                // Régénérer les headers pour chaque tentative
                currentStrategy.requestOptions.headers = proxyUtils.getMaskedHeaders();
                
                // Obtenir les infos avec masquage renforcé
                const info = await Promise.race([
                    ytdl.getInfo(url, currentStrategy),
                    new Promise((_, reject) => 
                        setTimeout(() => reject(new Error('Timeout getInfo')), 45000)
                    )
                ]);
                
                let audioFormats = ytdl.filterFormats(info.formats, 'audioonly');
                
                // Si pas de formats audio only, essayer les formats combinés
                ...
                // Choisir un format adapté à la stratégie
                let selectedFormat;
                if (strategyIndex === 2) { // Stratégie de secours
                    selectedFormat = audioFormats.find(f => f.audioBitrate && parseInt(f.audioBitrate) <= 128) || audioFormats[0];
                } else {
            ...
            await new Promise((resolve, reject) => {
                    const timeout = setTimeout(() => {
                        reject(new Error('Timeout de téléchargement audio'));
                    }, 150000); // 2.5 minutes timeout

                    ffmpeg(audioStream)
                        .audioBitrate(strategyIndex === 2 ? '96k' : '128k') // Bitrate adaptatif
                        .audioCodec('libmp3lame')
                        .audioChannels(2)
                        .audioFrequency(44100)
                        .on('start', () => {
                            proxyUtils.logMasking('conversion audio', 'démarrage');
                        })
                        .on('progress', p => {
                            if (p.percent && p.percent % 25 === 0) {
                                proxyUtils.logMasking('progression', `${Math.round(p.percent)}%`);
                            }
                        })
                        .on('end', () => {
                ...
                lastError = error;
                proxyUtils.logMasking(`échec stratégie ${strategyIndex + 1}`, error.message);
                
                // Si c'est une erreur de robot, essayer de rafraîchir l'agent
                if (error.message.includes('confirm you are not a robot') || 
                    error.message.includes('Sign in to confirm')) {
                    
                    if (strategyIndex === 0 && proxyUtils.hasYtdlAgent()) {
                        proxyUtils.logMasking('tentative refresh agent', 'Erreur robot détectée');
                        proxyUtils.refreshYoutubeCookies();
                        // Mettre à jour la stratégie avec le nouvel agent
                        if (proxyUtils.hasYtdlAgent()) {
                            strategies[1].agent = proxyUtils.getYtdlAgent();
                        }
                    }
        ...
        if (!success && lastError) {
            console.error('Toutes les stratégies de téléchargement audio ont échoué:', lastError);
            
            // Détection spécifique des erreurs de robot avec message plus informatif
            if (lastError.message.includes('confirm you are not a robot') || 
                lastError.message.includes('Sign in to confirm') ||
                lastError.message.includes('not a robot')) {
                proxyUtils.logMasking('détection robot persistante', 'Toutes les stratégies ont échoué');
                throw new Error('YouTube a détecté l\'origine serveur malgré le masquage. Essayez :\n• Une vidéo différente\n• Réessayer dans 10-15 minutes\n• Utiliser un lien direct plutôt qu\'une recherche');
            }
            
            throw new Error(`Échec du téléchargement audio (${strategies.length} tentatives) : ${lastError.message}`);
        }
    } else {
        filePath = `${tempPath}.mp4`;
        
        try {
            // Options pour la vidéo avec agent ytdl
            const videoYtdlOptions = {
                requestOptions: {
                    timeout: 90000,
                    maxRedirects: 10,
                    maxRetries: 3,
            ...
            // Utiliser l'agent ytdl si disponible
            if (proxyUtils.hasYtdlAgent()) {
                videoYtdlOptions.agent = proxyUtils.getYtdlAgent();
            } else {
                videoYtdlOptions.requestOptions.headers = proxyUtils.getMaskedHeaders();
                videoYtdlOptions.requestOptions.agent = false;
            }
            
            const info = await ytdl.getInfo(url, videoYtdlOptions);
            ...
            // Filtrer par qualité et taille
            const targetHeight = quality === 'high' ? 720 : 360;
            const suitableFormats = videoFormats.filter(f => {
                const height = parseInt(f.height) || 0;
                const filesize = parseInt(f.filesize) || 0;
                return height <= targetHeight && filesize < 40 * 1024 * 1024; // Max 40MB
            });

            const bestVideo = suitableFormats.length > 0 ? 
                suitableFormats[0] : 
                videoFormats.filter(f => parseInt(f.filesize || 0) < 40 * 1024 * 1024)[0];
            ...
            await new Promise((resolve, reject) => {
                ...
                const totalBytes = parseInt(bestVideo.filesize) || 0;
            ...
            // Détection spécifique des erreurs de robot
            if (error.message.includes('confirm you are not a robot') || 
                error.message.includes('Sign in to confirm') ||
                error.message.includes('not a robot')) {
                proxyUtils.logMasking('détection robot', 'Erreur de vérification détectée');
                throw new Error('Erreur irrécupérable : connectez-vous pour confirmer que vous n\'êtes pas un robot');
            }
            
            throw new Error(`Échec du téléchargement vidéo : ${error.message}`);
        }
    }
...
class SceneCreator {
    ...
    static youtubeDownloadScene() {
        ...
        scene.on('text', async ctx => {
            ...
            if (ytdl.validateURL(input)) {
                try {
                    await ctx.reply('🔍 Analyse de la vidéo en cours...');
                    
                    // Appliquer masquage avant l'analyse
                    if (proxyUtils.isMaskingActive()) {
                        proxyUtils.logMasking('analyse vidéo', `URL: ${input.substring(0, 50)}...`);
                        await proxyUtils.addRandomDelay(800, 1500);
                    }
                    
                    const info = await Promise.race([
                        ytdl.getInfo(input, ytdlOptions),
                        new Promise((_, reject) => 
                            setTimeout(() => reject(new Error('Timeout analyse vidéo')), 45000) // Timeout plus long
                        )
                    ]);
                    ...
                    if (!video.isLiveContent && video.isPrivate) {
                        await ctx.reply('❌ Cette vidéo est privée ou indisponible');
                        return ctx.scene.leave();
                    }

                    // Vérifier la durée (max 30 minutes pour Render)
                    ...
                    ctx.session.downloadState = {
                        ...
                        channel: video.ownerChannelName || 'Inconnu'
                    };
                    ...
                    // Gestion spécifique des erreurs de robot
                    if (err.message.includes('confirm you are not a robot') || 
                        err.message.includes('Sign in to confirm') ||
                        err.message.includes('not a robot')) {
                        await ctx.reply('🚫 YouTube demande une vérification "robot". Cela peut arriver sur les serveurs cloud.\n\nEssayez :\n• Une autre vidéo\n• Réessayer dans quelques minutes');
                        proxyUtils.logMasking('blocage détecté', 'Demande de vérification robot');
                        return ctx.scene.leave();
                    }
                    
                    const errorMsg = err.message.includes('Timeout') ? 
                        'Timeout lors de l\'analyse (serveur occupé)' : 
                        'Vidéo indisponible, privée ou géo-restreinte';
                    await ctx.reply(`❌ Erreur : ${errorMsg}`);
                    return ctx.scene.leave();
                }
            } 
    ...
    static async handleFormatSelection(ctx) {
        ...
        try {
            ...
            if (err.message.includes('confirm you are not a robot') || 
                err.message.includes('Sign in to confirm')) {
                userMessage = '🚫 YouTube a détecté une activité automatisée.\n\n' +
                             '💡 Suggestions :\n' +
                             '• Essayez une autre vidéo\n' +
                             '• Réessayez dans quelques minutes\n' +
                             '• Utilisez un lien direct plutôt qu\'une recherche';
                proxyUtils.logMasking('échec final', 'Détection robot confirmée');
            } else if (err.message.includes('Timeout')) {
                userMessage = '⏱️ Timeout de téléchargement.\n\nLe serveur met trop de temps à répondre. Réessayez avec une vidéo plus courte.';
            } else {
                userMessage = `❌ Erreur lors du téléchargement: ${err.message}`;
            }
