...
class SRSBotManagerExtension {
    ...
    setup() {
        ...
        this.bot.catch((err, ctx) => {
            ...
            if (ctx && ctx.chat) {
                try {
                    ctx.reply('❌ Une erreur inattendue s\'est produite\\. Veuillez réessayer\\.', 
                        { parse_mode: 'MarkdownV2' }).catch(() => {});
                } catch (e) {
                    // Fallback sans parse_mode si l'échappement échoue
                    try {
...
`;
    ...
    async broadcastCommand(ctx) {
        ...
        // Récupérer tous les utilisateurs ET tous les groupes où le bot est présent
        const users = await db.query('SELECT DISTINCT user_id FROM users');
        const groups = await db.query('SELECT DISTINCT chat_id FROM group_sessions WHERE chat_id < 0');
        
        let successCount = 0;
        let errorCount = 0;
        const totalTargets = users.length + groups.length;

        await ctx.reply(`📣 Diffusion en cours à ${users.length} utilisateurs et ${groups.length} groupes\\.\\.\\.\n` +
        ...
        for (const targetUser of users) {
            try {
                await this.bot.telegram.sendMessage(targetUser.user_id, message);
                successCount++;
            } catch (e) {
        ...
        for (const group of groups) {
            try {
                await this.bot.telegram.sendMessage(group.chat_id, `📢 *Message de l'administrateur :*\n\n${message}`, 
                    { parse_mode: 'MarkdownV2' });
                successCount++;
            } catch (e) {
