...
const CONFIG = {
    ...
    MAX_DAILY_DOWNLOADS: 5 // Limite par utilisateur
};
...
async function downloadYouTube(url, format, quality, title) {
    const safeTitle = title.replace(/[^\w\s]/gi, '').replace(/\s+/g, '_').substring(0, 50);
...
async function getUserDownloadCount(userId) {
...
async function recordUserDownload(userId) {
    const today = new Date().toISOString().split('T')[0];
    await db.run(
        `INSERT INTO user_downloads (user_id, date) VALUES (?, ?)`,
        [userId, today]
    );
}
...
class SceneCreator {
    static createVerseScene() {
        ...
        scene.on('text', async ctx => {
            ...
            try {
                ...
                await ctx.reply(`✅ Verset *${Utils.escapeMarkdown(verseName)}* créé avec succès !`, 
    ...
    static deleteVerseScene() {
        ...
        scene.on('text', async ctx => {
            ...
            try {
                ...
                await ctx.reply(`✅ Verset *${Utils.escapeMarkdown(verseName)}* supprimé avec succès !`, 
                    { parse_mode: 'MarkdownV2' }
                );
            } catch (err) {
    ...
    static youtubeDownloadScene() {
        ...
        scene.enter(async ctx => {
            ...
            if (downloadCount >= CONFIG.MAX_DAILY_DOWNLOADS) {
                await ctx.reply(
                    `🚫 Vous avez atteint votre limite quotidienne de téléchargements (${CONFIG.MAX_DAILY_DOWNLOADS}).\n` +
        ...
        scene.on('text', async ctx => {
            ...
            if (ytdl.validateURL(input)) {
                try {
                    ...
                    if (parseInt(video.lengthSeconds) > 3600) {
                    ...
                    await ctx.reply('❌ Erreur : Lien YouTube invalide ou vidéo indisponible');
                }
            } 
    ...
    static async showFormatOptions(ctx) {
        ...
        await ctx.replyWithPhoto(thumbnail, {
            ...
            reply_markup: {
                inline_keyboard: [
                    [
                        { text: '🎵 Audio (Haute qualité)', callback_data: 'format_audio_high' },
                        { text: '🎵 Audio (Standard)', callback_data: 'format_audio_std' }
                    ],
                    [
                        { text: '🎬 Vidéo (720p)', callback_data: 'format_video_720' },
...
class SRSBotManagerExtension {
    ...
    setup() {
        ...
        this.bot.catch((err, ctx) => {
            ...
            // Ignorer les erreurs de callback query expirées
    ...
    async startCommand(ctx) {
        ...
        const welcomeMsg = `
✨ *Bienvenue sur CDSanimeBase !* ✨

🧠 *Système avancé de flashcards et quiz pour fans d'anime*
📚 Créez, organisez et révisez vos connaissances sur vos personnages préférés
${isAdmin}
...
`;
        ...
        await ctx.reply(Utils.escapeMarkdown(welcomeMsg), { parse_mode: 'MarkdownV2' });
    }
...
`;
    ...
    async checkDownloadsCommand(ctx) {
        const userId = ctx.from.id;
        const downloadCount = await getUserDownloadCount(userId);
        const remaining = CONFIG.MAX_DAILY_DOWNLOADS - downloadCount;
        ...
        await ctx.reply(
            `📊 *Vos statistiques de téléchargement*\n\n` +
            `• Téléchargements aujourd'hui: *${downloadCount}*\n` +
            `• Restants: *${remaining}*\n` +
            `• Limite quotidienne: *${CONFIG.MAX_DAILY_DOWNLOADS}*\n\n` +
            `🔄 Réinitialisation dans: *${timeLeft} heures*`,
            { parse_mode: 'MarkdownV2' }
        );
    }
    ...
    async broadcastCommand(ctx) {
        ...
        await ctx.reply(`📣 Diffusion en cours à ${users.length} utilisateurs...`);
        ...
        for (const user of users) {
            try {
                ...
                console.error(`Erreur de diffusion à ${user.user_id}:`, e.message);
                errorCount++;
            }
