...
async function searchYouTube(query) {
    ...
    if (proxyUtils.isMaskingActive()) {
        ...
        await proxyUtils.addRandomDelay(300, 1000); // Délai court pour la recherche
    }
...
async function downloadYouTube(url, format, quality, title) {
    ...
    // Options avec masquage renforcé
    const ytdlOptions = {
        requestOptions: {
            headers: proxyUtils.getMaskedHeaders(),
            timeout: 60000 // Timeout plus long pour Render
        },
        highWaterMark: 1024 * 1024, // 1MB chunks
        quality: 'highest' // Laisser ytdl choisir
    };

    if (format === 'audio') {
        filePath = `${tempPath}.mp3`;
        
        // Méthode plus robuste pour Render
        try {
            ...
            // Choisir le meilleur format audio disponible
            ...
            await new Promise((resolve, reject) => {
                ...
                ffmpeg(audioStream)
                    .audioBitrate('128k') // Bitrate réduit pour Render
                    ...
                    .on('progress', p => {
                        console.log(`Audio progress: ${p.percent || 0}%`);
                    })
                    .on('end', () => {
            ...
            throw new Error(`Échec du téléchargement audio : ${error.message}`);
        }
    } else {
