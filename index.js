...
async function downloadYouTube(url, format, quality, title) {
    ...
    // Options optimisées pour Render
    const ytdlOptions = {
        requestOptions: {
            headers: {
        ...
        highWaterMark: 1024 * 1024 // 1MB chunks pour éviter les timeouts
    };
...
class SceneCreator {
                ...
                try {
                    ...
                    const info = await Promise.race([
                        ...
                        new Promise((_, reject) => 
                            setTimeout(() => reject(new Error('Timeout analyse vidéo')), 30000)
                        )
                    ]);
                    ...
                    if (audioFormats.length === 0 && videoFormats.length === 0) {
                        await ctx.reply('❌ Aucun format téléchargeable disponible pour cette vidéo');
                        return ctx.scene.leave();
                    }
                    ...
                    await ctx.reply(`❌ Erreur : ${err.message.includes('Timeout') ? 'Timeout lors de l\'analyse' : 'Vidéo indisponible ou restreinte'}`);
                    return ctx.scene.leave();
                }
            } 
    ...
    static async showFormatOptions(ctx) {
        const { title, duration, thumbnail, views, channel, hasAudio, hasVideo } = ctx.session.downloadState;
        ...
        // Ajouter les options audio si disponibles
        if (hasAudio) {
            keyboard.push([
        ...
        // Ajouter les options vidéo si disponibles
        if (hasVideo) {
            keyboard.push([
        ...
        keyboard.push([{ text: '🚫 Annuler', callback_data: 'format_cancel' }]);
