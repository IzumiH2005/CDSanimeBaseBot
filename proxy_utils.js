...
class ProxyUtils {
    ...
    /**
     * Charge les cookies YouTube depuis le fichier
     */
    loadYoutubeCookies() {
        const fs = require('fs');
        ...
        try {
            ...
            const cookieString = this.parseCookieFile(cookieContent);
            this.logMasking('cookies chargés', `${cookieString.length} caractères`);
            return cookieString;
        } catch (error) {
    ...
    /**
     * Parse le fichier de cookies Netscape vers un string Cookie
     */
    parseCookieFile(content) {
